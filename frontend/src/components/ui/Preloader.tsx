...
export default function Preloader({ onComplete }: PreloaderProps) {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 w-screen h-screen flex items-center justify-center bg-[#0f172a] z-[99999999999]"
    >
      {dimension.width > 0 && (
        <>
          <motion.p
          ...
          <svg className="absolute top-0 w-full h-[calc(100%+300px)]">
            <motion.path variants={curve} initial="initial" animate={isExiting ? "exit" : "initial"} fill="#0f172a" />
          </svg>
        </>
      )}
    </motion.div>
